...
describe('AuthService - generateRefreshToken', ()=>{
  ...
  it('should not accept old refresh token for refresh', async () => {
    ...
    const userRefreshed = await authService.refreshAccessToken(userCreated.refreshToken);
    
    expect(userRefreshed.refreshToken).not.toEqual(userCreated.refreshToken);
    expect(authService.refreshAccessToken(userCreated.refreshToken)).rejects.toThrow(
      ERRORS.AUTH.REFRESH_TOKEN_INVALID().message
    )
  });
})